...
Issues and PRDs for this repo live as GitHub issues in `WhatTheStackConf/wts`. Use the `gh` CLI for all operations.
...
External PRs are not pulled into the normal `/triage` queue. Triage GitHub Issues only unless the user explicitly asks to review or triage a PR.
