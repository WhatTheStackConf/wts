...
The adapter issues GET only, refuses redirects, uses no cookies, bounds each response to 2 MiB and each read to 10 seconds, and performs no automatic retry. Pagination must prove contiguous pages, constant totals/page size, expected row counts, unique IDs, matching page/path/first/last/prev/next and safe same-endpoint navigation links. It builds URLs locally, never follows an upstream next URL. Each collection is capped at 1,000 rows and 40 pages. Questions are a bounded complete collection. Missing metadata, duplicate/unsafe IDs, partial reads and cross-event product/question references fail closed. A dependency failure is `unavailable`; failure after a validated page is `partial`; neither returns a successful partial catalogue.
...
No item above was performed by these software tests. Production/device readiness, coordinator intake, rate-budget coordination for admission workloads, physical devices and rollout approval remain outside this slice. See [checkin-verification.md](checkin-verification.md) for the disposable automated gate.
