...
export async function fetchHiEventsAttendees(
  ...
  "use server";
  const { requireAdmin, requireAuth } = await import("~/lib/admin-security");

  if (!filterEmail) {
    // Fetching all attendees requires admin
    await requireAdmin();
  } else {
    // Fetching by email: user can only look up their own tickets
