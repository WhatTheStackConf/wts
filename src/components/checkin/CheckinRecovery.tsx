...
function RecoveryPanel(props: { workflowId?: string; admin: boolean; compact?: boolean; unavailable?: boolean; onBusyChange?: (busy: boolean) => void }) {
  ...
  const printerBlocked = () => blocked() || work()?.printerOperationsAllowed === false;
  ...
  async function submit(action?: Action) {
    ...
    if (action && ["observe", "replace", "handwrite", "release_isolation"].includes(action.operation) && printerBlocked()) return;
  ...
  return <section aria-label={props.admin ? "Admin recovery" : "Station label recovery"} class={props.compact ? "recovery-compact min-w-0 space-y-4 break-words" : "wts-name-label-text min-w-0 space-y-4 rounded-lg border border-base-content/20 p-4 break-words"}>
    ...
    <Show when={!props.compact}><p>Admission stays at its original station and event. Each label attempt records its actual printer destination. Parking does not make uncertain printing safe. No action here repeats a possibly-sent admission. A confirmed safe-read retry only renews pre-send work; normal readiness and send fences still apply.</p></Show>
    ...
    <Show when={!props.unavailable && !denied() && work()}>{w => <div class="space-y-4">
      ...
      <p>Label printer: {w().stationId}<Show when={w().admissionStationId && w().admissionStationId !== w().stationId}> · Admission station: {w().admissionStationId}</Show></p>
      <Show when={w().printerOperationsAllowed === false}><p role="status">Choose printer {w().stationId} before observing output, replacing, handwriting or releasing isolation.</p></Show>
      ...
      <Show when={!props.admin} fallback={<div class="space-y-3">
        ...
        <div class="space-y-3">
          <Show when={latest()}>{p => <><p>Latest output: {p().state} · observation: {p().observation || "none"} · cancellation: {p().cancellation || "none"}</p><div class="flex flex-wrap gap-2"><button class="btn min-h-12" disabled={printerBlocked() || !["completed", "uncertain"].includes(p().state) || !!p().observation} onClick={() => void submit({ operation: "observe", printId: p().id, outcome: "printed" })}>I observed printed output</button><button class="btn min-h-12" disabled={printerBlocked() || !["completed", "uncertain"].includes(p().state) || !!p().observation} onClick={() => void submit({ operation: "observe", printId: p().id, outcome: "not_printed" })}>I observed no usable output</button></div><p>Observation never automatically prints. Missing or damaged output needs a separate replacement request.</p></>}</Show>
          ...
          <button class="btn btn-warning min-h-12" disabled={printerBlocked() || w().isolated || !latest() || !["completed", "uncertain"].includes(latest()!.state) || (latest()!.state === "uncertain" && !latest()!.observation) || name() !== w().name || affiliation() !== w().affiliation} onClick={() => void submit({ operation: "replace", printId: latest()!.id })}>Request a new replacement label</button>
