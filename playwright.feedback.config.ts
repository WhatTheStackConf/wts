...
if (!state?.disposable || new URL(state.baseURL).hostname !== "127.0.0.1") throw new Error("Run pnpm test:feedback-browser with its disposable fixture.");
export default defineConfig({
  testDir: "./tests", testMatch: "feedback.spec.ts", fullyParallel: false, workers: 1, retries: 0,
  forbidOnly: Boolean(process.env.CI), timeout: 60_000, expect: { timeout: 15_000 }, reporter: "list",
  outputDir: "test-results/feedback",
