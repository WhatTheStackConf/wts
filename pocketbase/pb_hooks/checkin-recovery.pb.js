...
routerAdd("POST", "/api/wts/checkin-recovery", (e) => {
 ...
 e.app.runInTransaction(app=>{
  ...
  function own(w){if(!binding||binding.getBool("revoked")||(binding.getString("station")!==w.getString("station_id")&&!find(app,"checkin_print_attempts","workflow_id = {:workflow} && station_id = {:station}",{workflow:w.id,station:binding.getString("station")})))fail("forbidden",403);}
  function printStation(w){const p=R.latestPrint(app,w);return p?p.getString("station_id"):w.getString("station_id");}
  function ownPrint(w){if(!binding||binding.getBool("revoked")||binding.getString("station")!==printStation(w))fail("forbidden",403);}
  // SQL bounds the candidate IDs before hydration, including destination-only work.
  function historyCandidates(skip){const rows=new DynamicModel({ids:""});app.db().newQuery("SELECT COALESCE(json_group_array(id), '[]') AS ids FROM (SELECT w.id FROM checkin_arrival_workflows w WHERE w.edition = 'WTS2026' AND ({:admin} = 1 OR w.station_id = {:station} OR EXISTS (SELECT 1 FROM checkin_print_attempts p WHERE p.workflow_id = w.id AND p.station_id = {:station})) ORDER BY w.created DESC,w.id DESC LIMIT 100 OFFSET {:skip})").bind({admin:admin?1:0,station:binding?binding.getString("station"):"",skip}).one(rows);return JSON.parse(rows.ids).map(key=>app.findRecordById("checkin_arrival_workflows",key));}
  ...
  function dto(w,selectedPrints){const r=R.projection(app,w),latest=R.latestPrint(app,w),station=latest?latest.getString("station_id"):w.getString("station_id"), window=selectedPrints?null:app.findRecordsByFilter("checkin_print_attempts","workflow_id = {:id}","-created,-id",26,0,{id:w.id}), ps=selectedPrints||window.slice(0,25).reverse();if(!selectedPrints&&latest){const index=ps.findIndex(p=>p.id===latest.id);if(index>=0)ps.splice(index,1);else if(ps.length===25)ps.shift();ps.push(latest);}return {printerOperationsAllowed:!!binding&&!binding.getBool("revoked")&&binding.getString("station")===station,admissionReadRetryEligible:!!retryableAdmission(w),attemptsTruncated:!!window&&window.length>25,workflowId:w.id,stationId:station,admissionStationId:w.getString("station_id"),eventId:w.getString("event_id"),eventTitle:w.getString("event_title"),admissionState:w.getString("state"),version:r?r.getInt("version"):0,name:r?r.getString("name"):w.getString("name"),affiliation:r?r.getString("affiliation"):w.getString("affiliation"),decision:r?r.getString("decision"):"",fulfillment:r?r.getString("fulfillment"):"",parked:r?r.getBool("parked"):false,completedDay:r?r.getString("completed_day"):w.getString("admission_completed_day"),isolated:R.isolated(app,station),profile:json(latest||w,"profile_snapshot"),attempts:ps.map(p=>{const o=find(app,"checkin_recovery_observations","print_id = {:id}",{id:p.id}),c=find(app,"checkin_recovery_cancellations","print_id = {:id}",{id:p.id});return {id:p.id,stationId:p.getString("station_id"),profileId:p.getString("profile_id"),purpose:p.getString("purpose"),state:p.getString("state"),name:p.getString("name"),affiliation:p.getString("affiliation"),predecessorId:p.getString("predecessor_attempt_id"),observation:o?o.getString("outcome"):null,cancellation:c?c.getString("state"):null};}),reads:admin?app.findRecordsByFilter("checkin_recovery_reads","workflow_id = {:id}","-created,-id",20,0,{id:w.id}).map(r=>({id:r.id,state:r.getString("state"),checkinId:r.getString("checkin_id"),createdAt:r.getString("created")})):[],admissionAttempts:admin?app.findRecordsByFilter("checkin_arrival_attempts","workflow_id = {:id}","created,id",20,0,{id:w.id}).map(a=>({id:a.id,state:a.getString("state"),listId:a.getString("list_id"),attendeeId:a.getString("upstream_attendee_id"),sendBoundaryAt:a.getString("send_boundary_at"),completedAt:a.getString("completed_at"),fingerprint:a.getString("result_fingerprint")})):[],resets:admin?app.findRecordsByFilter("checkin_recovery_resets","workflow_id = {:id}","created,id",20,0,{id:w.id}).map(r=>({id:r.id,state:r.getString("state"),checkinId:r.getString("checkin_id"),sendBoundaryAt:r.getString("send_boundary_at")})):[],operationsEnabled:false};}
  if(machine){
  ...
  if(!truth){if(["observe","replace","handwrite","release_isolation"].includes(c.operation))ownPrint(w);else own(w);} // Printer attestations require the exact destination, including admins.
  ...
  const ps=prints(w),last=R.latestPrint(app,w),station=last?last.getString("station_id"):w.getString("station_id");
  ...
  function createPrint(purpose){const origin=purpose==="replacement"?last:w,p=require(__hooks+"/checkin-print-intent.js").create(app,w,{stationId:origin.getString("station_id"),profile:json(origin,"profile_snapshot"),purpose,name:r.getString("name"),affiliation:r.getString("affiliation"),predecessorId:purpose==="replacement"?last.id:""});r.set("latest_print_id",p.id);r.set("fulfillment","queued");r.set("completed_day","");}
  function cancel(){let pending=false;for(const p of ps){const t=terminal(p);if(t.settled)continue;let q=find(app,"checkin_recovery_cancellations","print_id = {:id}",{id:p.id});if(!q){q=new Record(app.findCollectionByNameOrId("checkin_recovery_cancellations"));set(q,{workflow_id:w.id,print_id:p.id,agent_attempt_id:t.a?t.a.id:"",station_id:p.getString("station_id"),state:t.a?"pending":"acknowledged",acknowledged_at:t.a?"":iso});app.save(q);}if(q.getString("state")!=="acknowledged")pending=true;}return pending;}
  ...
  if(c.operation==="retry_admission_reads"){
   ...
   const p=currentPrint();if(require(__hooks+"/checkin-print-intent.js").replacementState(app,w,p)!=="ready")fail("conflict",409);
   ...
   if(c.physicallyIsolated&&!R.isolated(app,station)){const isolation=new Record(app.findCollectionByNameOrId("checkin_recovery_isolations"));set(isolation,{station_id:station,workflow_id:w.id,command_id:command.id});app.save(isolation);}
   // An attestation at this printer cannot settle another printer's physical output.
   const foreignUnsafe=ps.some(p=>p.getString("station_id")!==station&&["dispatched","uncertain"].includes(p.getString("state"))&&!find(app,"checkin_recovery_observations","print_id = {:id}",{id:p.id}));
   const foreignPending=ps.some(p=>p.getString("station_id")!==station&&find(app,"checkin_recovery_cancellations","print_id = {:id} && state != 'acknowledged'",{id:p.id}));
   if(confirming&&!foreignUnsafe&&(!pending||(c.physicallyIsolated&&!foreignPending))){set(r,{fulfillment:"handwritten",completed_day:day(now)});}
   else{set(r,{fulfillment:"handwrite_pending",completed_day:""});}
  }else if(c.operation==="release_isolation"){
   if(c.confirmed!==true)fail("invalid_input");if(find(app,"checkin_recovery_cancellations","station_id = {:id} && state != 'acknowledged'",{id:station}))fail("conflict",409);
